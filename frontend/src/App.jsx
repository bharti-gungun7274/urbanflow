...
// Validation requests go directly to the FastAPI backend.
// This avoids the Vite frontend server swallowing /api/points/.../validate.
...
function RasterCanvas({ raster, pointNumber = 1, type = "lulc" }) {
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      console.error("URBANFLOW: createImageData failed", error);
      return;
    }
    ...
    if (!isReference) {
      ...
      for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
          ...
          const classValue = Number.isFinite(value) ? value : 255;
          const rgb = colors[classValue] || [255, 255, 255];

          imageData.data[pixel] = rgb[0];
          imageData.data[pixel + 1] = rgb[1];
          imageData.data[pixel + 2] = rgb[2];
          imageData.data[pixel + 3] = classValue === 255 ? 0 : 255;
        }
      }
    } else {
      ...
      const enhance = (value) => {
        const normalized = value / displayMaximum;
      ...
      for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
          ...
          const red = Number(redBand[y]?.[x] ?? 0);
          const green = Number(greenBand[y]?.[x] ?? 0);
          const blue = Number(blueBand[y]?.[x] ?? 0);

          imageData.data[pixel] = enhance(red);
          imageData.data[pixel + 1] = enhance(green);
          imageData.data[pixel + 2] = enhance(blue);
    ...
    if (
      ...
      pointX = Number(raster.point_column);
      pointY = Number(raster.point_row);
    } else if (
      ...
      pointX =
        Number(raster.column) - Number(raster.window.col_off);
      pointY =
        Number(raster.row) - Number(raster.window.row_off);
    } else {
    ...
    pointX = Math.max(
      0,
      Math.min(width - 1, Math.round(pointX))
    );

    pointY = Math.max(
      0,
      Math.min(height - 1, Math.round(pointY))
    );
    ...
    ctx.beginPath();
    ctx.moveTo(pointX - 18, pointY);
    ctx.lineTo(pointX + 18, pointY);
    ctx.moveTo(pointX, pointY - 18);
    ctx.lineTo(pointX, pointY + 18);
    ctx.stroke();
  ...
  if (!raster) {
    return <div className="viewer-empty">No raster loaded</div>;
  }

  const width = Math.max(
    1,
    Number(raster.width ?? raster.window?.width ?? 1)
  );

  const height = Math.max(
    1,
    Number(raster.height ?? raster.window?.height ?? 1)
  );
    ...
    <header className="header">
      <div className="brand">
        ...
        <div className="brand-text">
          <div className="brand-title">URBANFLOW</div>
      ...
      <nav className="navigation">
        <button
          className={page === "validation" ? "nav-active" : ""}
...
function FileSelector({ label, file, setFile, accept }) {
  return (
...
function PointDrawer({
  ...
  const validatedCount = points.filter(isValidatedPoint).length;

  return (
    <>
      {open && (
        <div className="point-status-backdrop" onClick={onClose} />
      )}

      {open && (
        <aside className="point-status-panel" aria-label="Validation point status">
          <div className="point-drawer-header">
            <div>
              <strong>VALIDATION STATUS</strong>
              <span>{validatedCount} / {points.length} checked</span>
            </div>
            <button
              type="button"
          ...
          <div className="drawer-progress">
            <div
              style={{
                width:
                  points.length > 0
                    ? `${(validatedCount / points.length) * 100}%`
                    : "0%",
              }}
            />
          </div>
          ...
          <div className="drawer-summary">
            <span className="status-key"><i className="status-dot done" /> Validated</span>
            <span className="status-key"><i className="status-dot undone" /> Not validated</span>
          </div>

          <div className="drawer-list">
            {points.length === 0 ? (
              <div className="drawer-empty">No validation points loaded.</div>
            ) : (
              points.map((point, index) => {
                const validated = isValidatedPoint(point);
                ...
                return (
                  <button
                    ...
                    className={`drawer-point ${current ? "current" : ""} ${validated ? "done" : "undone"}`}
                    onClick={() => {
...
function ValidationPage() {
  const [area, setArea] = useState("Agra");
  ...
  const [lulcRaster, setLulcRaster] = useState(null);
  const [referenceRaster, setReferenceRaster] = useState(null);

  const [referenceClass, setReferenceClass] = useState(null);
  const [referenceSource, setReferenceSource] = useState("Sentinel-2");
  ...
  async function handleLoadProject() {
    if (!lulcFile || !referenceFile || !pointsFile) {
      setMessage(
    ...
    try {
      ...
      const safePoints = Array.isArray(loadedPoints)
        ? loadedPoints
        : [];
      ...
      console.error("URBANFLOW project load error:", error);
  ...
  async function loadCurrentPoint(point) {
    if (!point) {
      setLulcRaster(null);
      setReferenceRaster(null);
      setReferenceClass(null);
      setReferenceSource("Sentinel-2");
      return;
    }
    ...
    try {
      ...
      const [lulc, reference] = await Promise.all([
        getRasterWindow(
          "lulc",
          longitude,
          latitude,
      ...
      setLulcRaster(lulc);
      setReferenceRaster(reference);

      /*
       * IMPORTANT:
       * For an unvalidated point, Dynamic World is the
       * automatic default reference class.
       *
       * For a previously validated point, preserve the
       * saved reference class.
       */
      const validated = isValidatedPoint(point);

      const dwClass = Number(point.dw_class);
      const savedReference =
        point.reference_class !== null &&
        point.reference_class !== undefined
          ? Number(point.reference_class)
          : null;
      ...
      setReferenceClass(
        validated &&
          ...
          ? savedReference
          : Number.isFinite(dwClass)
          ? dwClass
          : null
      );

      setReferenceSource(
        validated && point.reference_source
          ? point.reference_source
          : "Sentinel-2"
      );
    } catch (error) {
      console.error("URBANFLOW raster error:", error);
  ...
  useEffect(() => {
    ...
    loadCurrentPoint(currentPoint);
  }, [currentIndex, points, area, year]);
  ...
  useEffect(() => {
    ...
    async function recoverProject() {
      try {
        ...
        const recoveredPoints = await getPoints();

        if (
          cancelled ||
          !Array.isArray(recoveredPoints) ||
        ...
        const recoveredArea =
          info.study_area || "Agra";

        const recoveredYear =
          String(info.year || "2018");
        ...
        const recoveredIndex =
          Number.isInteger(savedIndex)
            ? Math.max(
                ...
                Math.min(
                  savedIndex,
                  recoveredPoints.length - 1
                )
              )
            : 0;

        setArea(recoveredArea);
        setYear(recoveredYear);
  ...
  async function savePoint(goNext = false) {
...
const validateResponse = await fetch(
  `${API_BASE_URL}/points/${currentPoint.index}/validate`,
  {
    method: "POST",
    headers: {
    ...
    mode: "cors",
    body: JSON.stringify({
      reference_class: Number(referenceClass),
      ...
      const validateData =
        await validateResponse.json().catch(() => ({}));

      if (!validateResponse.ok) {
        throw new Error(
          validateData?.detail ||
            "Failed to mark point as validated."
        );
      }
      ...
      setMessage(
        `Point ${currentIndex + 1} saved and validated.`
      );
      ...
      setMessage(
        error?.message ||
          "Failed to save validation point. Check that the backend is running on port 8000."
      );
    }
  }
  ...
  async function handleGoogleEarth() {
    if (!currentPoint) return;

    try {
      const result = await openGoogleEarth(
        currentPoint.index
      );

      setMessage(
        result?.message ||
          "Point opened in Google Earth Pro."
      );
    } catch (error) {
      console.error(
        "URBANFLOW Google Earth error:",
        error
      );
  ...
  return (
    <div className="page">
      <PointDrawer
        ...
        onClose={() =>
          setPointDrawerOpen((value) => !value)
        }
        points={points}
        ...
        <section className="sidebar-section compact-section validation-control-section">
          ...
          <div className="point-navigation">
            ...
            <div className="point-count">
              Point {points.length ? currentIndex + 1 : 0}
              {" / "}
              {points.length}
            </div>
          ...
          >
            <span>VIEW VALIDATION STATUS</span>
            <strong>
              {validatedCount}/{points.length}
            </strong>
          </button>
        </section>
            ...
            <section className="sidebar-section compact-section">
              <div className="section-title-row">
                ...
                <span className="auto-reference-badge">
                  {currentPoint.dw_class !== undefined
...
function ResultsPage({ setPage, active }) {
  ...
  async function handleExport() {
    try {
      const result = await exportValidation();
    ...
    <main className="results-page">
      <div className="results-heading">
        <div>
          ...
          <p>
            {results.total_samples} validated samples
          </p>
        </div>
